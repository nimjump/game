...
var _error_banner  : Control = null  # error banner sliding from top
...
func _build_game() -> void:
	...
	share_btn.pressed.connect(func():
		...
		# session_id dolana kadar kısa süre bekle (backend submit async, max 3s)
		var _waited := 0
		while _session_id == "" and _waited < 15:
			await get_tree().create_timer(0.2).timeout
			_waited += 1
		var replay_url : String = ""
		if _session_id != "":
			replay_url = ApiConfig.GAME_URL + "?replay=" + _session_id
		var msg : String = "I scored %d in NimJump! Watch my replay: %s" % [sc, replay_url] if replay_url != "" else "I scored %d in NimJump! Can you beat me? %s" % [sc, ApiConfig.GAME_URL]
		ApiConfig.share_score(sc, msg)
	)
...
func _build_start_ui() -> void:
	...
	_ui_root.add_child(_error_banner)
...
func _build_quest_panel() -> void:
	...
	_stats_panel.connect("replay_requested", func(seed: int, log: PackedByteArray, char_idx: int, nick: String):
		await _start_replay(seed, log, char_idx, "stats")
	)
...
func _build_settings_popup() -> void:
	...
	var close_sz := int(_p(0.048))  # QuestPanel ile aynı boyut, ic'ten bağımsız
	...
	close_ic2.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	close_btn.add_child(close_ic2)
	...
	var _make_sound_row := func(label: String, enabled: bool, volume: float,
		...
		var _row_icon_path := func(is_on: bool, vol: float) -> String:
			...
			if not is_on: return at2.get("icon_sound_off", "")
			if vol < 0.5:
...
func _open_nick_overlay() -> void:
	...
	x_btn.custom_minimum_size = Vector2(int(_p(0.048)), int(_p(0.048)))
	_warm_btn_st(x_btn)
	...
	x_ic.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	x_btn.add_child(x_ic)
...
func _on_leaderboard_replay_requested(seed: int, replay_log: PackedByteArray, char_idx: int, _nickname: String) -> void:
	if _started: return
	if _block_lb_replay: return
	...
	await _start_replay(seed, replay_log, char_idx, "leaderboard")
...
#  ERROR BANNER
# ─────────────────────────────────────────────────────
func _make_error_banner() -> Control:
	var banner := PanelContainer.new()
	banner.visible = false
	banner.set_anchor_and_offset(SIDE_LEFT,   0, 0)
	banner.set_anchor_and_offset(SIDE_RIGHT,  1, 0)
	banner.set_anchor_and_offset(SIDE_TOP,    0, -_p(0.12))
	banner.set_anchor_and_offset(SIDE_BOTTOM, 0,  _p(0.12))
	var sb := StyleBoxFlat.new()
	sb.bg_color = Color(0.85, 0.15, 0.15, 0.92)
	...
	UITheme.apply_label(lbl, Color.WHITE, int(_p(0.030)))
