...
func _build_ui() -> void:
	...
	close_btn.custom_minimum_size = Vector2(ic, ic)
...
func _build_list(data: Variant) -> void:
	...
	for i in entries.size():
		...
		if has_replay_e and session_id_e != "":
			...
			rp_btn.text = ""
			var _play_ic_path : String = UITheme.get_theme_assets().get("icon_play", "")
			if ResourceLoader.exists(_play_ic_path):
				rp_btn.icon = load(_play_ic_path)
				rp_btn.expand_icon = true
				rp_btn.add_theme_constant_override("icon_max_width", int(ref * 0.032))
			else:
				rp_btn.text = "PLAY"
				rp_btn.add_theme_font_size_override("font_size", int(ref * 0.022))
			rp_btn.custom_minimum_size = Vector2(int(ref * 0.055), int(ref * 0.040))
			rp_btn.modulate = UITheme.COL_ORANGE
			_warm_ghost_btn(rp_btn, 6)
			rp_btn.pressed.connect(_fetch_and_emit_replay.bind(session_id_e, rp_btn))
			player_cell.add_child(rp_btn)

		var score_lbl := _make_col_label(str(score), ref, 0.22, _C_BROWN, false)
		score_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
		row.add_child(score_lbl)

		var prize_str := (str(snappedf(float(prize), 0.0001))) if prize > 0 else "0"
		var prize_lbl := _make_col_label(prize_str, ref, 0.13, _C_BROWN if prize > 0 else _C_MID, false)
...
func _fetch_and_emit_replay(session_id_e: String, btn: Button) -> void:
	...
	http.request_completed.connect(func(result, code, _h, body):
		...
		btn.disabled = false
		btn.text = ""
		var _pip : String = UITheme.get_theme_assets().get("icon_play", "")
		if ResourceLoader.exists(_pip): btn.icon = load(_pip); btn.expand_icon = true
		else: btn.text = "PLAY"
		...
		replay_requested.emit(seed, log_bytes, char_idx, nickname)
		hide_panel()
		closed.emit()
	)
