...
func _poll() -> void:
	...
	# _nimiqAddress dolana kadar max 30 saniye bekle
	for _w in 300:
		var a1 = JavaScriptBridge.eval("window._nimiqAddress || ''", true)
		var a2 = JavaScriptBridge.eval("window._nimiqAddress || ''", false)
		var a1s := str(a1) if a1 != null else ""
		var a2s := str(a2) if a2 != null else ""
		if (a1s != "" and a1s != "null") or (a2s != "" and a2s != "null"):
			break
		await get_tree().create_timer(0.1).timeout

	# Adresi oku
	var addr_raw = JavaScriptBridge.eval("window._nimiqAddress || ''", true)
	if addr_raw == null or str(addr_raw) == "" or str(addr_raw) == "null":
		addr_raw = JavaScriptBridge.eval("window._nimiqAddress || ''", false)
		...
		if code == 200:
			...
			if j.parse(body.get_string_from_utf8()) == OK:
				...
				if d.get("ok", false):
					...
					if secs_left < 86400:
						print("[NimiqBridge] Token expires soon (%ds) — will re-sign" % secs_left)
						JavaScriptBridge.eval("localStorage.removeItem('nj_auth_token')", true)
						JavaScriptBridge.eval("localStorage.removeItem('nj_auth_pid')", true)
					...
					auth_success.emit(auth_token, auth_player_id)
					return
		# Backend says token invalid (401, etc.) — clear it
...
func _verify_with_backend(challenge: String, public_key: String, signature: String) -> void:
	...
	http.request_completed.connect(func(result, code, _h, body):
		http.queue_free()
		if result != HTTPRequest.RESULT_SUCCESS or code != 200:
			var err: String = "verify_failed_%d" % code
