...
func _build_ui() -> void:
	...
	var vh  := vp.get_visible_rect().size.y if vp else 900.0
	var ref := minf(vw, vh)
	...
	close_btn.custom_minimum_size = Vector2(int(ref * 0.048), int(ref * 0.048))
...
func _build_quest_list() -> void:
	_clear_list()
	var ref := minf(get_viewport().get_visible_rect().size.x, get_viewport().get_visible_rect().size.y)
...
func _show_error(msg: String) -> void:
	...
	row.add_theme_constant_override("separation", int(ref * 0.010))
	_list_root.add_child(row)
	row.add_child(UITheme.lucide_icon("alert-triangle", int(ref * 0.034), Color(0.480, 0.340, 0.200)))
	var lbl := Label.new()
	lbl.text = msg
	lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
...
func _show_toast(msg: String) -> void:
	...
	row.add_theme_constant_override("separation", int(ref * 0.008))
	_list_root.add_child(row)
	row.add_child(UITheme.lucide_icon("alert-triangle", int(ref * 0.028), Color(0.820, 0.180, 0.120)))
	var lbl := Label.new()
	lbl.text = msg
	lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	UITheme.apply_label(lbl, Color(0.820, 0.180, 0.120), int(ref * 0.026))
	row.add_child(lbl)
	var tw := create_tween()
	tw.tween_interval(2.5)
	tw.tween_callback(func(): if is_instance_valid(row): row.queue_free())
