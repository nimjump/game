	...
	if player._initialized:
		...
		if _replay_mode == ReplayMode.PLAYING:
			...
			if _replay_mode == ReplayMode.RECORDING:
				...
				# Extend run or start new run
				if _replay_log.size() > 0 and _replay_log[_replay_log.size() - 1] != 0xFF:
...
func _add_spikes(plat: StaticBody2D) -> void:
	# Pattern seç: 0=klasik küme, 1=orta+iki kenar (boşluklu 3), 2=sadece orta
	var pattern := _rng.randi() % 3
	var side    := _rng.randi() % 2  # RNG tüketimi — determinizm için

	var headless := _is_headless
	var TARGET_H : float = VH * 0.0175
	var vis_h := TARGET_H
	var vis_w := TARGET_H * 0.8
	var sc := 1.0
	var spike_tex : Texture2D = null

	if not headless:
		spike_tex = _cached_tex("res://assets/environment/spike_top.png")
		if not spike_tex: return
		var tw := float(spike_tex.get_width()); var th := float(spike_tex.get_height())
		if tw <= 4.0 or th <= 4.0: return
		sc = TARGET_H / th
		vis_h = TARGET_H; vis_w = tw * sc

	# Pozisyon listesi belirle
	var positions : Array[float] = []
	var margin := PLATFORM_W * 0.08
	match pattern:
		0: # Klasik — bir kenarda 1-2 spike
			var spike_count := 1 + (side % 2)
			var cx := PLATFORM_W * 0.25 * (1.0 if side == 1 else -1.0)
			var spacing := vis_w * 1.1
			var sx := cx - spacing * (spike_count - 1) * 0.5
			for i in spike_count:
				positions.append(sx + i * spacing)
		1: # Ortada 1 + iki kenarda 1'er — aralarında boşluk
			positions.append(0.0)
			positions.append(-(PLATFORM_W * 0.5 - margin))
			positions.append( (PLATFORM_W * 0.5 - margin))
		2: # Sadece tam orta
			positions.append(0.0)
	...
	for x_pos in positions:
		...
		area.position = Vector2(x_pos, -(PLATFORM_H * 0.5) - vis_h * 0.5)
		area.z_index = 10
		plat.add_child(area)
		if not headless and spike_tex:
			var spr := Sprite2D.new()
			spr.texture = spike_tex; spr.scale = Vector2(sc, sc)
			spr.z_index = 10
			area.add_child(spr)
		var cs := ConvexPolygonShape2D.new()
		var hw := vis_w * 0.45
		var hh := vis_h * 0.5
		cs.points = PackedVector2Array([
			Vector2(0,   -hh),
			Vector2(-hw,  hh * 0.8),
			Vector2( hw,  hh * 0.8),
		])
		...
		_interactables.append({"area": area, "type": "spike", "data": "top", "used": false, "_cached": false, "_r": 0.0})
...
func _add_spike_bottom(plat: StaticBody2D) -> void:
	...
	var vis_h := 87.0 * sc
	var vis_w := 51.0 * sc
	# Pattern: 0=ortada tek, 1=iki kenarda eşit
	var pattern := _rng.randi() % 2
	var margin  := PLATFORM_W * 0.08
	var positions : Array[float] = []
	match pattern:
		0: positions.append(0.0)
		1:
			positions.append(-(PLATFORM_W * 0.5 - margin))
			positions.append( (PLATFORM_W * 0.5 - margin))
	...
	for x_pos in positions:
		...
		plat.add_child(area)
		if not headless:
			var tex := _cached_tex("res://assets/environment/spike_bottom.png")
			if tex:
				var spr := Sprite2D.new()
				spr.texture = tex
				spr.scale   = Vector2(sc, sc)
		...
		col.shape = cs
		area.add_child(col)
		_interactables.append({"area": area, "type": "spike", "data": "bottom", "used": false, "_cached": false, "_r": 0.0})
...
func _add_spring(plat: StaticBody2D) -> void:
	...
	var h_out := VH * 0.035  # approximate — used for collision offset in headless
	...
	if not headless:
		...
		if tex_out and tex_out.get_width() > 0:
			...
			h_out = float(tex_out.get_height()) * sc
	...
	match type:
		...
		"spring":
			...
			if not _is_headless:
				...
				# Reset spring after delay — visual only
				...
				# Headless: reset spring after fixed ticks via a flag
				# Spring reuse: just reset used_ref after ~32 ticks (0.53s)
		...
		"proj_damage":
			...
			if one_shot and is_instance_valid(area): area.queue_free()
		"rain_damage":
...
func start_replay() -> void:
	...
	while _rle_di < _replay_log.size():
		...
		if _rle_db == 0xFF:
			...
			continue
		_replay_total_ticks += max(1, (_rle_db >> 2) & 0x3F)
		_rle_di += 1
...
func seek_to_tick(target_tick: int) -> void:
	...
	_is_seeking = true
	const SEEK_BATCH := 500  # ticks per frame during seek
...
func _submit_session() -> void:
	if not _is_authed():
		print("[GM] _submit_session: guest — skipping submit")
		return
