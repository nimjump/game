...
Safari/WebKit's WASM threading implementation chokes on the pthread pool
size Godot's web export bakes in by default (a nonzero `pool_size`) — it
can throw or hang on load specifically on Safari/iOS, while working fine
on Chrome/Firefox. `patch_safari.py` (repo root, next to `game/` and
...
- Removes the Emscripten version-check `throw` some export versions bake
  in, which can hard-fail load in browsers Emscripten doesn't recognize.
- Forces `pool_size` to `0` — the actual Safari pthread-pool fix.

And in `export/index.html`:

- Forces `emscriptenPoolSize` / `godotPoolSize` to `0` (same pool-size
  fix, HTML-side config).
- If present, replaces an old inline fetch+inject `<script>` block with a
  plain `<script src="game.js">` tag.
- If present, cleans up a stale `_startGodotSetup` / `_godotEngineLoaded`
  trigger pattern into a direct `initGodotGame()` call.

It also deletes any stale `game.js.fasthttp.br` / `game.html.fasthttp.br`
brotli-cache files left over in `export/` from a previous build, so a new
export never accidentally gets served from an old cached copy.

Safe to re-run — every regex is written to only match if the
old/unpatched pattern is still there, so running it twice on an
already-patched export just prints 0 replacements instead of corrupting
anything. **Skip this and Safari/iOS users can fail to load the game
while everyone else is fine** — this is exactly the kind of "works for
me, broken for one platform" bug that's easy to miss if you only test on
Chrome.
...
5. Copies _headers, manifest.json, robots.txt, sitemap.xml, and the
   assets/ folder from game/template into export/, overwriting whatever
   was there. robots.txt/sitemap.xml have to land at export/'s top level
   because crawlers request them straight from the domain root
   (https://.../robots.txt) — they don't work sitting only in the source
   template folder.
...
- [ ] export/robots.txt and export/sitemap.xml present — generated by build.py from game/template; if the canonical domain ever changes, update it in game/template/index.html (canonical/OG/Twitter/JSON-LD tags), robots.txt, and sitemap.xml together — they all hardcode https://nimjump.zetashare.com
